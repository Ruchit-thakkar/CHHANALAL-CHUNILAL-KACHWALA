...
export default function Navbar({
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            >
              <span
                className={`font-heading text-base sm:text-lg md:text-xl font-bold tracking-tight uppercase transition-colors duration-300 ${
                  isScrolled ? "text-[#171717]" : "text-white lg:text-white"
                }`}
              >
                Chhanalal Chunilal Kachwala
              </span>
              <span
                className={`text-[10px] sm:text-[11px] tracking-[0.22em] uppercase font-medium transition-colors duration-300 ${
                  isScrolled ? "text-[#B99A63]" : "text-[#D4BD8E]"
                }`}
              >
            ...
            <nav className="hidden md:flex items-center space-x-7 lg:space-x-9" aria-label="Main Navigation">
              {navLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`text-sm font-medium transition-all duration-200 relative py-1 hover:text-[#B99A63] ${
                    isScrolled ? "text-[#171717]" : "text-white/90 hover:text-white"
                  }`}
                >
                  {link.label}
            ...
            <div className="hidden md:flex items-center space-x-3">
              <button
                ...
                className={`group inline-flex items-center justify-center text-xs uppercase tracking-wider font-semibold px-5 py-2.5 transition-all duration-300 border ${
                  isScrolled
            ...
            <div className="flex items-center space-x-2 md:hidden">
              <button
                onClick={() => onOpenQuoteModal()}
                className={`text-xs px-3 py-1.5 font-medium uppercase tracking-wider border ${
                  isScrolled
              ...
              <button
                ...
                className={`p-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B99A63] ${
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`fixed inset-0 z-40 bg-[#111111]/95 backdrop-blur-lg md:hidden transition-all duration-300 flex flex-col justify-between p-6 pt-24 ${
